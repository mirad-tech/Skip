...
这是手动安装的 Android APK，未上架应用商店。安装时如果系统提示“未知来源应用”，需要用户自行确认是否继续安装。
...
- 默认规则：内置并可编辑全局开屏跳过模板，包含关键词、View ID 关键词、时间窗、区域、分数和冷却策略。
...
- 默认只在应用进入前台后的 8 秒时间窗口内生效，可在默认规则设置中调整为 6、8 或 10 秒。
