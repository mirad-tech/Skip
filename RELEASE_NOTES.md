...
- 开屏时间窗外的窗口内容变化事件不再读取无障碍节点树。
- 开屏时间窗内，窗口内容变化事件最快每 300ms 扫描一次。
- 窗口状态变化和窗口列表变化仍进入完整处理，以便前台会话可以重启。
- 单次节点扫描以及输入框、坐标锚点、页面安全文本遍历最多访问 500 个节点。
- 时间窗外日志按前台会话最多记录一次。
- 保留 Android 13 及以上关闭框架无障碍缓存与预取。
...
- Release APK：`Skip-v1.0.16-release.apk`
...
- Android 13 及以上版本在服务连接后关闭框架无障碍节点缓存与预取，避免框架长期保留节点副本。
- 点击候选的父节点读取统一经过禁用预取的节点访问层，避免默认父节点接口隐式预取祖先、后代和同级节点。
- 保留现有事件与任务边界的缓存清理，作为低版本系统的兼容路径。

### 兼容性

- Android 12 及以下版本保留原有节点访问路径，不扩大自动点击范围、安全策略或权限边界。
...
- Release APK：`Skip-v1.0.15-release.apk`
...
- Android 13 及以上版本的节点根、子节点和父节点读取统一关闭框架预取，避免无障碍缓存持续扩张。
...
- 修复成功提示弹窗显示包名的问题：默认规则的应用名为空或等于包名时，会解析为系统应用名。
- 修复导出日志和诊断包的版本展示，新导出的日志会读取当前安装包的 `versionName`。
- 改善 B 站右上角小面积“关闭”按钮的窄范围识别，不降低全局阈值。
- 改善虎牙高置信内置跳过按钮的点击时机，降低候选节点在点击前消失的概率。
- 增强 vivo/Funtouch OS 等多窗口场景下的可见窗口选择和开屏期短时重扫。
- 忽略本地 `logs/` 目录，避免设备诊断数据误提交到仓库。

### 安全和隐私边界

- 仍然只做本地无障碍节点识别和点击，不新增联网、广告、统计或上传能力。
- 高风险按钮保护策略保持不变，不自动点击支付、授权、登录、注册、隐私同意、安装、删除、转账、发送、提交等按钮。
- 新增的交互窗口读取只用于当前可见窗口选择。
...
- Release APK：`Skip-v1.0.1-release.apk`
- SHA256：`455371B0F9EF18F915415DA0CC42956D3BCB32DDFC1DDF84A9305B454D3A180E`
